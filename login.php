...
if ($_SERVER["REQUEST_METHOD"]) {
    ...
    $stmt = $conn->prepare("SELECT * FROM fdm_users WHERE email = '$email' AND role = '$role'");
    ...
    if ($row && password_verify($password, $row['password'])) {
        ...
        $stmt = $conn->prepare( "UPDATE fdm_users SET otp = '$otp', otp_expiry = '$otp_expiry' WHERE email = '$email'");
        $stmt->bind_param("ss", $otp, $otp_expiry);
        $stmt->execute();
